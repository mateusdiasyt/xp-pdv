...
import {
  ...
  updateCategoryFiscalRulesRecord,
  updateCategoryStatusRecord,
} from "@/application/catalog/category-service";
...
export async function updateCategoryFiscalRulesAction(formData: FormData): Promise<ActionState> {
  try {
    const session = await requirePermission(PERMISSIONS.CATEGORIES_MANAGE);
    await updateCategoryFiscalRulesRecord(formData, session.user.id);
    return { status: "success", message: "Regra fiscal salva com sucesso." };
  } catch (error) {
    return { status: "error", message: toActionErrorMessage(error) };
  }
}
